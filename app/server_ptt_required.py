...
# ===========================================
# Cancelable Piper TTS Worker (separate process)
# ===========================================
class PiperTTSWorker:
    """
    Runs Piper synthesis in a separate Python process so we can hard-cancel mid-flight by terminating the process.
    Protocol (stdin/stdout):
      - parent sends one JSON line: {"cmd":"synth","lang":"en","text":"..."}
      - worker replies with JSON lines:
          {"type":"chunk","b64":"..."}  # base64 of mu-law 8k audio bytes (arbitrary chunk size)
          {"type":"done"}
          {"type":"err","msg":"..."}
    """
    ...
    def hard_cancel(self):
        # terminate immediately; next synth will auto-restart
        self.stop()

    def synthesize_stream(self, text_in: str, lang: str, cancel_evt: Optional[threading.Event] = None):
        """
        Yields mu-law 8k chunks from the worker. If cancel_evt is set, stops immediately and hard-cancels worker.
        """
        ...
        try:
            req = {"cmd": "synth", "lang": lang, "text": text_in}
            ...
            while True:
                ...
                if mtype == "chunk":
                    ...
                    # log worker error and stop
                    print(f"[ERROR] TTS worker: {msg.get('msg')}")
                    self.stop()
                    return
        except Exception as e:
...
# Create a singleton worker (path inside container is /app/tts_worker.py)
TTS_WORKER_PATH = os.getenv("TTS_WORKER_PATH", "/app/tts_worker.py").strip()
tts_worker = PiperTTSWorker(TTS_WORKER_PATH)
...
def ws(ws):
    ...
    def cancel_current_generation():
        with cancel_lock:
            cancel_evt.set()
        # hard-cancel any in-flight Piper synthesis
        try:
            tts_worker.hard_cancel()
        except Exception:
            pass
    ...
    def respond_to_utterance(utt_pcm16_8k: bytes, noise_pcm16_8k: bytes):
        ...
        for rchunk, reply_lang in stream_llm_reply(text, lang):
            ...
            if should_flush(tts_buffer):
                if cancel_evt.is_set():
                    break
                # Stream TTS from cancelable worker (hard-cancellable mid-synthesis)
                if stream_sid:
                    for mulaw_chunk in tts_worker.synthesize_stream(tts_buffer, current_tts_lang, cancel_evt=cancel_evt):
                        send_audio_to_twilio(ws, stream_sid, mulaw_chunk, send_lock=send_lock, cancel_event=cancel_evt)
                        if cancel_evt.is_set():
                            break
                tts_buffer = ""
        ...
        if not cancel_evt.is_set() and tts_buffer.strip():
            if stream_sid and not cancel_evt.is_set():
                for mulaw_chunk in tts_worker.synthesize_stream(tts_buffer, current_tts_lang, cancel_evt=cancel_evt):
                    send_audio_to_twilio(ws, stream_sid, mulaw_chunk, send_lock=send_lock, cancel_event=cancel_evt)
                    if cancel_evt.is_set():
                        break
...
<Response>
  ...
  <Connect>
    <Stream url="{stream_url}"><Parameter name="Dtmf" value="True" /><Parameter name="pttDigit" value="5" /></Stream>
  </Connect>
</Response>"""
